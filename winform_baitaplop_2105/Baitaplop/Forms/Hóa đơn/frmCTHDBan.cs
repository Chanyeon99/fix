...
namespace Baitaplop.Forms
{
    public partial class frmCTHDBan : Form
    {
        ...
        public void refresh()
        {
            ...
            using (SqlConnection cnn = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("select ChiTietHDB.smahdb,SanPham.stensp,ChiTietHDB.isoluong,ChiTietHDB.fgiamgia,SanPham.fgiaban,ChiTietHDB.fthanhtien from ChiTietHDB, SanPham where ChiTietHDB.smasp = SanPham.smasp and ChiTietHDB.smahdb='"+ frmHoaDonBan.shdb+"'", cnn))
                {
        ...
        public bool kiemtratensp()
        {
            ...
            {
                ktsp = false;
                SqlDataAdapter da = new SqlDataAdapter("select * from ChiTietHDB where smasp='" + cbotensp.SelectedValue + "' and smahdb='" + frmHoaDonBan.shdb + "'", cnn);
        ...
        private void btnxoa_Click(object sender, EventArgs e)
        {
            ...
            if (tb == DialogResult.OK)
            {
                try
                {
                    ...
                    conn.Open();
                    string sql = "delete from ChitietHDB where smasp = '" + cbotensp.SelectedValue + "'and smahdb = '"+frmHoaDonBan.shdb+"'";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandText = sql;
                    cmd.ExecuteNonQuery();

                    string tongtien = "update HoaDonBan set ftongtien = (select sum(fdongian*isoluong-fgiamgia) from ChiTietHDB where smahdb='" + txtmhd.Text + "') where smahdb='" + frmHoaDonBan.shdb + "'";
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (themmoi == true)
                {
                    if (kiemtratensp())
                    {
                        MessageBox.Show("Tên Sản phẩm đã tồn tại trong hóa đơn có mã: "+ frmHoaDonBan.shdb+"!");
                    }
                    else
                    {
                        string constr = ConfigurationManager.ConnectionStrings["btl"].ConnectionString;
                        using (SqlConnection cnn = new SqlConnection(constr))
                        {
                            ...
                            using (SqlCommand cmd = cnn.CreateCommand())
                            {
                                ...
                                try
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@smhd", frmHoaDonBan.shdb);
                ...
                else
                {
                    try
                    {
                        ...
                        if (txtmhd.Text != "")
                        {
                            ...
                            string sql = "update ChiTietHDB set isoluong='" + txtsl.Text + "',fgiamgia='" + txtgg.Text + "',fdongian='" + txtdg.Text + "' where smahdb ='" + frmHoaDonBan.shdb + "' and smasp =N'" + cbotensp.SelectedValue + "'";
                            SqlCommand cmd = new SqlCommand(sql, conn);
                            cmd.CommandText = sql;
                            cmd.ExecuteNonQuery();
