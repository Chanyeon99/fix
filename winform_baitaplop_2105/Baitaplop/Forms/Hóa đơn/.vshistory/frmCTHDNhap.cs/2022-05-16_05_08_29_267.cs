...
namespace Baitaplop.Forms
{
    public partial class frmCTHDNhap : Form
    {
        ...
        public void refresh()
        {
            ...
            using (SqlConnection cnn = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("select ChiTietHDN.smahdn,SanPham.stensp,ChiTietHDN.isoluong,ChiTietHDN.fdongian,Hangsx.shangsx,ChiTietHDN.fthanhtien from ChiTietHDN,SanPham,Hangsx where ChiTietHDN.smasp=SanPham.smasp and Sanpham.smahangsx=Hangsx.smahangsx and ChiTietHDN.smahdn='" + frmCTHDNhap.shdn + "'", cnn))
                {
        ...
        private void Chi_tiết_frmCTHDNhap_Load(object sender, EventArgs e)
        {
            showcombobox();
            refresh();
            cbotensp.SelectedValue="";
            txtmhd.Text = frmCTHDNhap.shdn;
        }
        public bool kiemtratensp()
        {
            ...
            {
                ktsp = false;
                SqlDataAdapter da = new SqlDataAdapter("select * from ChiTietHDN where smasp='" + cbotensp.SelectedValue + "' and smahdn='" + frmCTHDNhap.shdn + "'", cnn);
        ...
        private void btnxoa_Click(object sender, EventArgs e)
        {
            ...
            if (tb == DialogResult.OK)
            {
                try
                {
                    ...
                    conn.Open();
                    string sql = "delete from ChitietHDN where smasp = '" + cbotensp.SelectedValue + "'and smahdn = '" + frmCTHDNhap.shdn + "'";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandText = sql;
                    cmd.ExecuteNonQuery();

                    string tongtien = "update HoaDonNhap set ftongtien = (select sum(fdongian*isoluong) from ChiTietHDN where smahdn='" + txtmhd.Text + "') where smahdn='" + frmCTHDNhap.shdn + "'";
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (themmoi == true)
                {
                    if (kiemtratensp())
                    {
                        MessageBox.Show("Tên Sản phẩm đã tồn tại trong hóa đơn có mã: " + frmCTHDNhap.shdn + "!");
                    }
                    else
                    {
                        string constr = ConfigurationManager.ConnectionStrings["btl"].ConnectionString;
                        using (SqlConnection cnn = new SqlConnection(constr))
                        {
                            ...
                            using (SqlCommand cmd = cnn.CreateCommand())
                            {
                                ...
                                try
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@mhdn", frmCTHDNhap.shdn);
                ...
                else
                {
                    try
                    {
                        ...
                        if (txtmhd.Text != "")
                        {
                            ...
                            string sql = "update ChiTietHDN set isoluong='" + txtsl.Text + "', shangsx='"+txthangsx.Text+"' where smahdn ='" + frmCTHDNhap.shdn + "' and smasp =N'" + cbotensp.SelectedValue + "'";
                            SqlCommand cmd = new SqlCommand(sql, conn);
                            cmd.CommandText = sql;
                            cmd.ExecuteNonQuery();
        ...
        private void txttk_TextChanged(object sender, EventArgs e)
        {
            ...
            using (SqlConnection cnn = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT ChiTietHDN.smahdn,SanPham.stensp,ChiTietHDN.isoluong,ChiTietHDN.fdongian,Hangsx.shangsx,ChiTietHDN.fthanhtien from ChiTietHDN,SanPham,Hangsx where ChiTietHDN.smasp=SanPham.smasp and Sanpham.smahangsx=Hangsx.smahangsx and ChiTietHDN.smahdn='" + frmCTHDNhap.shdn + "' and SanPham.stensp like N'%" + txttk.Text.ToString() + "%'", cnn))
                {
                    cnn.Open();
