...
namespace Baitaplop.Forms
{
    public partial class frmHoaDonNhap : Form
    {
        ...
        public void loadCTHDN(string shdn)
        {
            string constr = ConfigurationManager.ConnectionStrings["btl"].ConnectionString;
            using (SqlConnection cnn = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("select ChiTietHDN.smahdn,SanPham.stensp,ChiTietHDN.isoluong,ChiTietHDN.fdongian,Hangsx.shangsx,ChiTietHDN.fthanhtien from ChiTietHDN,SanPham,Hangsx where ChiTietHDN.smasp=SanPham.smasp and Sanpham.smahangsx=Hangsx.smahangsx and ChiTietHDN.smahdn='" + shdn + "'", cnn))
                {
                    cmd.CommandType = CommandType.Text;
                    cnn.Open();
                    using (SqlDataAdapter ad = new SqlDataAdapter(cmd))
